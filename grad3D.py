...
def CFD(n,p,h):
    #Takes the central finite difference between n and p, assuming equally 
    #spaced in negative and positive directions by step size h.
    return ((p-n)/(2*h))**2
...
def grad3DKernel(d_u,d_v,maxVal):
    i,j,k = cuda.grid(3)
    dims = d_u.shape
    if i<dims[0]-1 and j<dims[1]-1 and k<dims[2]-1 and min(i,j,k)>1:
        h1 = 1 #step size
        h2 = 2**(1/2)
