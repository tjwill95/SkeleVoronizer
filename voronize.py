...
def voronize(origObject, seedPoints, cellThickness, shellThickness,
             name = "", sliceLocation = 0, sliceAxis = "X", order = 2, smoothing = False):
    ...
    #wallThickness  = desired minimum thickness of cell walls (vx).
    #shellThickness = desired minimum thickness of shell (vx), 0 if no shell
    ...
    voronoi = f.thicken(voronoi,wallThickness)
    if smoothing:
        voronoi = f.smooth(SDF3D(voronoi))
    voronoi = f.intersection(voronoi,f.thicken(SDFOO,-min(shellThickness-1,4)))
    if name !="":
        slicePlot(voronoi, sliceLocation, titlestring=(name+' Trimmed and Thinned'),axis = sliceAxis)
    if shellThickness>0:
        u_shell = f.subtract(f.thicken(SDF3D(origObject),-shellThickness),origObject)
